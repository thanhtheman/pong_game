...
def run_neat(config):
    ...
    # we run the genomes function up to 50 times, we going to save the best genome into the pickle file
    winner = p.run(eval_genomes, 1)  
    with open("best.pickle", "wb") as f:
        pickle.dump(winner, f)

#test the best AI saved in the pickle file:
def test_ai(config):
    width, height = 700, 500
...
if __name__ == "__main__":
    ...
    # run_neat(config)
    #after we runneat() we will comment it out to test the best genome - the winner
    test_ai(config)